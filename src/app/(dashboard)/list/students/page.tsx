...
import { Eye, Trash2, Filter, ArrowUpDown, Plus, BookOpen, Users } from "lucide-react";
  ...
  return (
    <div className="flex-1 m-4 mt-0 flex flex-col gap-4">
      ...
      <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5 font-medium">{studentsData.length} members registered</p>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total Students", value: studentsData.length, icon: <Users size={16} />, color: "bg-indigo-50 text-indigo-600" },
          { label: "Subjects", value: 6, icon: <BookOpen size={16} />, color: "bg-amber-50 text-amber-600" },
          { label: "Active Classes", value: 12, icon: <Users size={16} />, color: "bg-emerald-50 text-emerald-600" },
          { label: "New This Month", value: 50, icon: <Plus size={16} />, color: "bg-violet-50 text-violet-600" },
        ].map((stat) => (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex-1">
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className="relative shrink-0">
                        <Image src={item.photo} alt={item.name} width={38} height={38}
                          className="w-9 h-9 rounded-xl object-cover ring-2 ring-gray-100" />
                        <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-white" />
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-sm text-gray-800 truncate">{item.name}</p>
                        <p className="text-xs text-gray-400 truncate">{item.class}</p>
                      </div>
                    </div>
                  </td>

                  <td className="px-3 py-3.5 hidden md:table-cell">
                    <span className="text-xs font-mono font-semibold text-gray-500 bg-gray-100 px-2 py-1 rounded-lg">{item.studentId}</span>
                  </td>
        ...
        <div className="border-t border-gray-100"><Pagination /></div>
      </div>
    </div>
  );
};
