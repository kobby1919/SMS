    ...
    <div className="flex-1 m-4 mt-0 flex flex-col gap-4">
      ...
      <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-black text-gray-800 tracking-tight">Parents</h1>
            <p className="text-sm text-gray-400 mt-0.5 font-medium">{parentsData.length} members registered</p>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total Parents", value: parentsData.length, icon: <Users size={16} />, color: "bg-indigo-50 text-indigo-600" },
          ...
          <table className="w-full min-w-[360px]">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/60">
                <th className="text-left px-4 py-3.5 text-xs font-black uppercase tracking-wider text-gray-400">Parent</th>
                <th className="text-left px-3 py-3.5 text-xs font-black uppercase tracking-wider text-gray-400 hidden md:table-cell">Student</th>
              ...
              {parentsData.map((item: Parent) => (
                <tr key={item.id} className="hover:bg-indigo-50/30 transition-colors duration-150 group">
                  ...
                  <td className="px-3 py-3.5 hidden md:table-cell">
                    <span className="text-sm text-gray-500">{item.students}</span>
                  </td>
