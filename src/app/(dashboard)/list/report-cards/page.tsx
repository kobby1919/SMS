...
const ReportCardListPage = async ({
  ...
  const supervisedClasses = role === "admin"
  ...
  const academicYears = configs.length > 0 ? configs.map((c) => c.academicYear) : ["2024/25", "2025/26"];
  const activeYear   = selectedYear || academicYears[0] || "2024/25";

  // ── Config for this year ──────────────────────────────────────────────────
  const config = await prisma.cAConfig.findUnique({ where: { academicYear: activeYear } });
  ...
  for (const l of lessons) {
    if (!subjectMap.has(l.subject.id)) subjectMap.set(l.subject.id, l.subject.name);
  }
  ...
  const studentRows: StudentRow[] = students.map((s) => {
    ...
    const top     = records.sort((a, b) => a.gradePoint - b.gradePoint)[0]?.grade ?? "—";
    ...
    <div className="flex-1 m-4 mt-0 flex flex-col gap-4">
      ...
      <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black text-gray-800 tracking-tight">Report Cards</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: "Complete",   value: completeCount,    color: "bg-emerald-50 text-emerald-600" },
          { label: "Pending",    value: students.length - completeCount, color: "bg-amber-50 text-amber-600" },
          { label: "Subjects",   value: totalSubjects,    color: "bg-violet-50 text-violet-600"  },
        ].map((s) => (
      ...
      {!config && (
        <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-2xl">
          ...
          <p className="text-xs font-semibold text-amber-700">
            No CA configuration found for <strong>{activeYear}</strong>. Report cards will use
            the default 30% / 70% split. Ask your admin to configure weights at{" "}
            <Link href="/admin/ca-config" className="underline">CA Settings</Link>.
          </p>
        </div>
      )}
        ...
        <div className="divide-y divide-gray-50">
          {sorted.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-gray-400 font-semibold">No students in this class</p>
            </div>
          ) : (
            sorted.map((s, idx) => {
              const position = idx + 1;
              const band     = s.totalScore > 0 ? getGradeBandByGrade(s.topGrade) : null;
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-sm font-black shrink-0
                    ${position === 1 && s.complete ? "bg-amber-100 text-amber-700"
                    : position === 2 && s.complete ? "bg-slate-100 text-slate-600"
                    : position === 3 && s.complete ? "bg-orange-100 text-orange-700"
                  ...
                  <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-sm font-black text-indigo-600 shrink-0">
                    {s.img
                      ? <img src={s.img} alt="" className="w-full h-full object-cover rounded-xl" />
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-800 text-sm">{s.surname} {s.name}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      {s.complete ? (
                        ...
                        <span className="flex items-center gap-1 text-[10px] font-bold text-amber-500">
                          <Clock size={10} /> {s.subjectsDone}/{totalSubjects} subjects
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  {s.subjectsDone > 0 && (
                    <div className="hidden sm:flex items-center gap-4">
                      <div className="text-right">
                        <p className="text-xs text-gray-400 font-semibold">Avg Score</p>
                        <p className="text-sm font-black text-gray-800">{s.totalScore}%</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-gray-400 font-semibold">Aggregate</p>
                        <p className="text-sm font-black text-gray-800">{s.aggregate}</p>
                      </div>
                      {band && (
                        <div className={`px-2.5 py-1 rounded-xl border text-xs font-black ${band.bg} ${band.color} ${band.border}`}>
                          {s.topGrade}
                        </div>
                      )}
                    </div>
                  ...
                  {s.complete && (
                    <div className="hidden md:block text-right">
                      ...
                      <p className="text-sm font-black text-gray-800">{ordinal(position)}</p>
                    </div>
                  )}
