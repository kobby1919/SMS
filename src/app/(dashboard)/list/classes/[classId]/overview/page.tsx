...
const ClassOverviewPage = async ({
  ...
  return (
    <div className="m-3 mt-0 flex min-w-0 flex-1 flex-col gap-4 sm:m-4 sm:mt-0">
      ...
      {role === "admin" && !overview.class.supervisorId ? (
        <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900 shadow-sm sm:p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0">
              <p className="text-sm font-black">No class teacher assigned yet</p>
              <p className="mt-1 text-sm font-semibold leading-6 text-amber-800">
                This class can still be reviewed by admin, but class-teacher privileges, class supervision, and report submission will stay incomplete until one teacher is assigned as the class teacher.
              </p>
            </div>
            <Link
              href={`/list/classes?search=${encodeURIComponent(overview.class.name)}`}
              className="inline-flex w-full items-center justify-center rounded-xl bg-amber-900 px-4 py-2.5 text-xs font-black text-white transition hover:bg-amber-800 sm:w-fit"
            >
              Go to class setup
            </Link>
          </div>
