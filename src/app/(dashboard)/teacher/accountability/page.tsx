...
function DutyRowItem({
  ...
  const isEscalated = needsManagementReview(row);
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          {(showType || row.escalationStatus) && !isEscalated && (
          ...
          {isEscalated ? (
            <span className="inline-flex rounded-full bg-rose-50 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-rose-700">
              Needs response
            </span>
          ) : null}
          <p className={(showType || row.escalationStatus || isEscalated) ? "mt-2 font-black text-slate-950" : "font-black text-slate-950"}>
            {cleanDutyTitle(row)}
          </p>
          ...
          {row.escalationReason && isEscalated ? (
            <p className="mt-2 rounded-lg bg-rose-50 px-3 py-2 text-sm font-semibold leading-relaxed text-rose-700">
              {row.escalationReason}
            </p>
          ) : null}
          {row.teacherResponseStatus ? (
            <p className="mt-2 rounded-lg bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-600">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <Link
            href={href}
            className="inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-slate-950 px-3 py-2.5 text-sm font-black text-white transition hover:bg-slate-700 sm:w-auto"
          >
...
function TodayDutiesPanel({
  ...
  const emptyMessage =
    escalatedTodayCount > 0
      ? "No normal duties left. Your remaining duty or duties have moved to escalation review above."
      : "No duty has been assigned to you today.";
...
const TeacherAccountabilityPage = async ({
  ...
  const completedToday = overview.todayDuties.filter(
    (row) => row.status === "COMPLETED" || row.status === "COMPLETED_LATE",
  ).length;
  const pendingToday = todayRows.filter((row) => row.status === "PENDING").length;
  ...
  return (
    <div className="flex flex-col gap-4 p-3 sm:gap-5 sm:p-4">
      <div className="rounded-lg border border-slate-200 bg-slate-950 p-4 text-white shadow-sm sm:p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          helper="All duties expected today"
          tone="blue"
        />
        <StatCard
          label="Completed"
        ...
        <StatCard
          label="Pending"
          value={pendingToday}
          helper="Still normal duties"
          tone={pendingToday > 0 ? "amber" : "green"}
        />
        <StatCard
          label="Needs Attention"
          value={attentionRows.length}
          helper="Moved to escalation review"
          tone={attentionRows.length > 0 ? "red" : "green"}
        />
      </div>
      <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-sm">
        <p className="text-sm font-semibold leading-relaxed text-slate-600">
          Today&apos;s total includes completed, pending, missed, and escalated duties.
          Escalated duties appear in the review panel so they are not mixed with normal work.
      ...
      <div className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
        <NeedsAttentionPanel
          rows={attentionRows}
          focusedObligationId={activeFocusedObligationId}
        />
        <TodayDutiesPanel
          rows={todayRows}
          focusedObligationId={activeFocusedObligationId}
          escalatedTodayCount={attentionRows.length}
        />
      </div>

      <HistorySection weeks={pastHistoryWeeks} />
    </div>
  );
};
