...
import { CheckCircle2, Clock3, Mail, ShieldCheck, UsersRound, XCircle } from "lucide-react";
import {
  getParentInvitePreview,
  type ParentInvitePreview,
} from "@/src/lib/services/parent-invites";
import ParentInviteAcceptButton from "@/src/components/ParentInviteAcceptButton";
import { normalizeAppRole } from "@/src/lib/roles";
import InviteSignOutButton from "@/src/components/InviteSignOutButton";

type ParentAcceptInvitePageProps = {
  searchParams: Promise<{ token?: string }>;
};

const stateCopy: Record<
  ParentInvitePreview["state"],
  { label: string; title: string; body: string }
> = {
  active: {
    label: "Parent invite",
    title: "Join your ward's school on Edujay",
    body: "Continue with the exact email address this invite was sent to. Edujay will connect your account only to the ward records approved by the school.",
  },
  accepted: {
    label: "Invite already accepted",
    title: "This invite has already been used",
    body: "This parent invite is already connected to an Edujay account. If you need access, ask the school admin to check your parent profile.",
  },
  expired: {
    label: "Invite expired",
    title: "This invite has expired",
    body: "For security, parent invites expire after a short period. Ask the school admin to resend your invite.",
  },
  revoked: {
    label: "Invite revoked",
    title: "This invite is no longer active",
    body: "The school has revoked this invite. Ask the school admin to issue a fresh parent invite if you still need access.",
  },
  invalid: {
  ...
  missing: {
    ...
    title: "No parent invite token was found",
...
export default async function ParentAcceptInvitePage({
  searchParams,
}: ParentAcceptInvitePageProps) {
  const { token } = await searchParams;
  const [invite, user] = await Promise.all([
    getParentInvitePreview(token),
    currentUser(),
  ]);
  ...
  const roleConflict = Boolean(invite.usable && signedInRole && signedInRole !== "parent");
  ...
  const signInHref = `/sign-in?parentInvite=${encodeURIComponent(token ?? "")}`;
      ...
      <section className="mx-auto flex w-full max-w-[920px] flex-col gap-6 lg:flex-row lg:items-stretch">
        <div className="flex-1 rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-2xl sm:p-8">
          ...
          {roleConflict && (
            <div className="mt-6 rounded-xl border border-amber-300/25 bg-amber-300/10 p-4">
              ...
              <p className="mt-2 text-sm font-medium leading-6 text-amber-50/70">
                This parent invite must be accepted by the parent&apos;s own account.
                Sign out first, then open the invite link as the parent.
              </p>
              <InviteSignOutButton redirectUrl={signInHref} label="Sign out and use parent email" />
            </div>
          )}
          ...
          {emailMatches && (
            <div className="mt-6 rounded-xl border border-emerald-300/25 bg-emerald-300/10 p-4">
              ...
              <p className="mt-2 text-sm font-medium leading-6 text-emerald-50/70">
                You are signed in with the invited email. Edujay will now connect this account to the approved ward records.
              </p>
            </div>
          )}
          ...
          {invite.usable && !emailMatches && !mustSwitchAccount ? (
            ...
            <ParentInviteAcceptButton token={token ?? ""} />
          ) : (
            <Link
              href="/"
              className="mt-8 inline-flex w-full items-center justify-center rounded-xl border border-white/10 px-5 py-3 text-sm font-black text-white/80 transition hover:bg-white/10 sm:w-auto"
        ...
        <aside className="w-full rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-2xl lg:max-w-[340px]">
          ...
          <div className="mt-5 space-y-4">
            ...
            <Detail label="Parent" value={invite.parentName ?? "Not available"} />
            <Detail label="Email" value={invite.email ?? "Not available"} icon={<Mail size={15} />} />
            <Detail
              label="Wards"
              value={invite.wardNames?.length ? invite.wardNames.join(", ") : "Not available"}
            ...
            <div className="rounded-xl border border-white/10 bg-black/20 p-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-xs font-semibold leading-5 text-white/55">
                  Edujay verifies the signed-in email, invite status, school, and ward links before creating parent access.
                </p>
              </div>
            </div>
          </div>
