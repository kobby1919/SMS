...
import { CheckCircle2, Clock3, Mail, ShieldCheck, WalletCards, XCircle } from "lucide-react";
import {
  getBursarInvitePreview,
  type BursarInvitePreview,
} from "@/src/lib/services/bursar-invites";
import BursarInviteAcceptButton from "@/src/components/BursarInviteAcceptButton";
import { normalizeAppRole } from "@/src/lib/roles";
import InviteSignOutButton from "@/src/components/InviteSignOutButton";

type BursarAcceptInvitePageProps = {
  searchParams: Promise<{ token?: string }>;
};

const stateCopy: Record<
  BursarInvitePreview["state"],
  { label: string; title: string; body: string }
> = {
  active: {
    label: "Bursar invite",
    title: "Join your school finance team on Edujay",
    body: "Continue with the exact email address this invite was sent to. Edujay will use it to securely connect your account to the school finance workspace.",
  },
  accepted: {
    label: "Invite already accepted",
    title: "This invite has already been used",
    body: "This bursar invite is already connected to an Edujay account. If you need access, ask the school admin to check your bursar profile.",
  },
  expired: {
    label: "Invite expired",
    title: "This invite has expired",
    body: "For security, bursar invites expire after a short period. Ask the school admin to resend your invite.",
  },
  revoked: {
    label: "Invite revoked",
    title: "This invite is no longer active",
    body: "The school has revoked this invite. Ask the school admin to issue a fresh bursar invite if you still need access.",
  },
  invalid: {
  ...
  missing: {
    ...
    title: "No bursar invite token was found",
    body: "Open the invite link sent by your school admin, or ask them to send a fresh invite.",
  },
};

export default async function BursarAcceptInvitePage({
  searchParams,
}: BursarAcceptInvitePageProps) {
  const { token } = await searchParams;
  const [invite, user] = await Promise.all([
    getBursarInvitePreview(token),
    currentUser(),
  ]);
  ...
  const roleConflict = Boolean(invite.usable && signedInRole && signedInRole !== "bursar");
  ...
  const signInHref = `/sign-in?bursarInvite=${encodeURIComponent(token ?? "")}`;
      ...
      <section className="mx-auto flex w-full max-w-[920px] flex-col gap-6 lg:flex-row lg:items-stretch">
        <div className="flex-1 rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-2xl sm:p-8">
          ...
          {emailMismatch && (
            <div className="mt-6 rounded-xl border border-amber-300/25 bg-amber-300/10 p-4">
              <p className="text-sm font-black text-amber-100">Signed in with the wrong email</p>
              ...
              <InviteSignOutButton redirectUrl={signInHref} label="Sign out and use invited email" />
            </div>
          )}
          ...
          {roleConflict && (
            <div className="mt-6 rounded-xl border border-amber-300/25 bg-amber-300/10 p-4">
              ...
              <p className="mt-2 text-sm font-medium leading-6 text-amber-50/70">
                This bursar invite must be accepted by the bursar&apos;s own account.
                Sign out first, then open the invite link as the bursar.
              </p>
              <InviteSignOutButton redirectUrl={signInHref} label="Sign out and use bursar email" />
            </div>
          )}

          {emailMatches && (
            <div className="mt-6 rounded-xl border border-emerald-300/25 bg-emerald-300/10 p-4">
              <p className="text-sm font-black text-emerald-100">Email verified</p>
              <p className="mt-2 text-sm font-medium leading-6 text-emerald-50/70">
                You are signed in with the invited email. Edujay will now connect this account to the school finance workspace.
              </p>
            </div>
          )}
          ...
          {invite.usable && !emailMatches && !mustSwitchAccount ? (
            ...
            <BursarInviteAcceptButton token={token ?? ""} />
          ) : (
            <Link
              href="/"
              className="mt-8 inline-flex w-full items-center justify-center rounded-xl border border-white/10 px-5 py-3 text-sm font-black text-white/80 transition hover:bg-white/10 sm:w-auto"
        ...
        <aside className="w-full rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-2xl lg:max-w-[340px]">
          ...
          <div className="mt-5 space-y-4">
            ...
            <Detail label="Bursar" value={invite.bursarName ?? "Not available"} icon={<WalletCards size={15} />} />
            ...
            <div className="rounded-xl border border-white/10 bg-black/20 p-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-xs font-semibold leading-5 text-white/55">
                  Edujay verifies the signed-in email, invite status, school, and finance role before creating bursar access.
                </p>
              </div>
            </div>
          </div>
