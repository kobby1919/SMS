...
import {
  resendBursarInviteAction,
  revokeBursarInviteAction,
  type BursarInviteActionResult,
} from "@/src/lib/actions/bursarInviteActions";

export default function BursarInviteActions({ inviteId }: { inviteId: string }) {
  ...
  function handleResult(result: BursarInviteActionResult, fallback: string) {
    if (!result.ok) {
  ...
  function resend() {
    ...
    startTransition(async () => {
      const result = await resendBursarInviteAction({ inviteId });
      handleResult(result, "Invite resent. Copy the new link if needed.");
    });
  }

  function revoke() {
    if (!window.confirm("Revoke this bursar invite? The current invite link will stop working.")) {
      return;
    }
    ...
    startTransition(async () => {
      const result = await revokeBursarInviteAction({ inviteId });
