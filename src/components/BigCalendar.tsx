...
const COLOR_PALETTE = [
  { bg: "bg-blue-50",    text: "text-blue-700",    border: "border-l-blue-400",    dot: "bg-blue-400"    },
  { bg: "bg-amber-50",   text: "text-amber-700",   border: "border-l-amber-400",   dot: "bg-amber-400"   },
  { bg: "bg-emerald-50", text: "text-emerald-700", border: "border-l-emerald-400", dot: "bg-emerald-400" },
  { bg: "bg-violet-50",  text: "text-violet-700",  border: "border-l-violet-400",  dot: "bg-violet-400"  },
  { bg: "bg-rose-50",    text: "text-rose-700",    border: "border-l-rose-400",    dot: "bg-rose-400"    },
  { bg: "bg-orange-50",  text: "text-orange-700",  border: "border-l-orange-400",  dot: "bg-orange-400"  },
  { bg: "bg-teal-50",    text: "text-teal-700",    border: "border-l-teal-400",    dot: "bg-teal-400"    },
  { bg: "bg-pink-50",    text: "text-pink-700",    border: "border-l-pink-400",    dot: "bg-pink-400"    },
  { bg: "bg-cyan-50",    text: "text-cyan-700",    border: "border-l-cyan-400",    dot: "bg-cyan-400"    },
  { bg: "bg-lime-50",    text: "text-lime-700",    border: "border-l-lime-400",    dot: "bg-lime-400"    },
  { bg: "bg-indigo-50",  text: "text-indigo-700",  border: "border-l-indigo-400",  dot: "bg-indigo-400"  },
  { bg: "bg-fuchsia-50", text: "text-fuchsia-700", border: "border-l-fuchsia-400", dot: "bg-fuchsia-400" },
];

const buildColorMap = (lessons: CalendarLesson[]) => {
  const unique = Array.from(new Set(lessons.map((l) => l.title)));
  return Object.fromEntries(
    unique.map((subject, i) => [subject, COLOR_PALETTE[i % COLOR_PALETTE.length]])
  );
};
...
export type CalendarLesson = {
  ...
  startTime:  Date;
  endTime:    Date;
  className?: string;   // shown for teacher view
  teacher?:   string;   // shown for student/parent view
};
...
// ── KEY FIX: filter (not find) — returns ALL lessons at that hour ──────────────
const getLessonsForDay = (lessons: CalendarLesson[], dayName: string) =>
  lessons.filter((l) => dayEnumToLabel[l.day] === dayName);

const getLessonsAtHour = (lessons: CalendarLesson[], dayName: string, hour: number) =>
  lessons.filter(
    (l) =>
...
// ── Compact card used in week view grid cells ─────────────────────────────────
const LessonCardCompact = ({
  lesson,
  colorMap,
}: {
  lesson: CalendarLesson;
  colorMap: Record<string, (typeof COLOR_PALETTE)[0]>;
}) => {
  const c = colorMap[lesson.title] ?? COLOR_PALETTE[0];
  return (
    <motion.div
      whileHover={{ scale: 1.02 }}
      className={`rounded-xl border-l-4 px-2 py-1.5 flex flex-col gap-0.5 cursor-default ${c.bg} ${c.border}`}
    >
      <span className={`font-bold text-[11px] leading-snug ${c.text}`}>
        {lesson.title}
      </span>
      {lesson.className && (
        <span className={`text-[9px] font-semibold ${c.text} opacity-75`}>
          📚 {lesson.className}
        </span>
      )}
      {lesson.teacher && (
      ...
      <span className={`text-[9px] font-medium opacity-60 ${c.text}`}>
...
// ── Full card used in day view ────────────────────────────────────────────────
const LessonCardFull = ({
  lesson,
  colorMap,
}: {
  lesson: CalendarLesson;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <span className={`font-bold text-sm ${c.text}`}>{lesson.title}</span>
            {lesson.className && (
              <span className={`text-[11px] ml-2 ${c.text} opacity-70`}>
                · 📚 {lesson.className}
              </span>
            )}
            {lesson.teacher && (
        ...
        <span className={`text-[11px] font-semibold shrink-0 ml-4 sm:ml-0 ${c.text} opacity-70`}>
          {formatTime(lesson.startTime)} – {formatTime(lesson.endTime)}
          {" "}· {getDuration(lesson.startTime, lesson.endTime)}
        </span>
      </div>
        ...
        {viewMode === "week" && (
          <motion.div key="week" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }} className="w-full overflow-x-auto">
            <div className="min-w-[640px] bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                {days.map((day) => (
                  <div key={day} className="p-3 text-center border-l border-gray-100">
                    ...
                    <p className="text-[10px] text-gray-300 mt-0.5">
                      {getLessonsForDay(lessons, day).length} period{getLessonsForDay(lessons, day).length !== 1 ? "s" : ""}
                    </p>
                  </div>
                ))}
              </div>
              ...
              {/* Hour rows */}
              {hours.map((hour, hi) => (
                <div key={hour} className="grid border-b border-gray-50 last:border-b-0" style={{ gridTemplateColumns: "64px repeat(5, 1fr)" }}>
                  ...
                  {days.map((day) => {
                    // ── FIXED: filter not find ──
                    const dayLessons = getLessonsAtHour(lessons, day, hour);
                    return (
                      <div key={day} className={`border-l border-gray-50 min-h-[72px] p-1 flex flex-col gap-1 ${hi % 2 !== 0 ? "bg-gray-50/40" : ""}`}>
                        {dayLessons.map((lesson, i) => (
                          <LessonCardCompact key={i} lesson={lesson} colorMap={colorMap} />
                        ))}
                      </div>
                    );
                  })}
          ...
          <motion.div key={`day-${selectedDay}`} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.22 }} className="w-full">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 mt-0.5">
                    {getLessonsForDay(lessons, selectedDay).length} period{getLessonsForDay(lessons, selectedDay).length !== 1 ? "s" : ""}
              ...
              <div className="divide-y divide-gray-50">
                {hours.map((hour) => {
                  // ── FIXED: filter not find — all lessons at this hour ──
                  const hourLessons = getLessonsAtHour(lessons, selectedDay, hour);
                  return (
                    <div key={hour} className="flex items-stretch min-h-[64px]">
                      ...
                      <div className="flex-1 px-3 py-2 flex flex-col gap-2 justify-center min-w-0">
                        {hourLessons.length > 0 ? (
                          hourLessons.map((lesson, i) => (
                            <LessonCardFull key={i} lesson={lesson} colorMap={colorMap} />
                          ))
                        ) : (
