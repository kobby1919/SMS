...
import { reviewAttendanceCorrectionRequest } from "@/src/lib/actions/actions";

export default function AttendanceCorrectionReviewActions({
  requestId,
}: {
  requestId: string;
}) {
  ...
  const review = (action: "APPROVE" | "REJECT") => {
    ...
    startTransition(async () => {
      try {
        ...
        setError(caught instanceof Error ? caught.message : "Could not review attendance correction.");
      } finally {
