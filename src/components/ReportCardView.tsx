...
//
// fix: replace window.print() with real server-side PDF download
//   The "Download Report Card" button now hits /api/report-card/pdf
//   which generates a proper A4 PDF using @react-pdf/renderer on the server.
//   The page still renders beautifully in the browser for screen viewing.

import { useState } from "react";
import {
  Download, ArrowLeft, Award, TrendingUp,
  CheckCircle2, AlertCircle, Star, Loader2,
} from "lucide-react";
...
function PosBadge({ pos }: { pos: number }) {
  ...
  return <span className={`text-xs font-black ${pos <= 3 ? "text-amber-600" : "text-gray-500"}`}>{ordinal(pos)}</span>;
}

// ─── Download button ──────────────────────────────────────────────────────────
function DownloadButton({
  studentId, term, academicYear, classId,
}: {
  studentId: string; term: string; academicYear: string; classId?: number;
}) {
...
const ReportCardView = ({
  ...
  const narrative    = getPerformanceNarrative(
    overallStats.aggregate, overallStats.avgScore,
    overallStats.overallPosition, overallStats.classSize,
  );
  ...
  const classId = typeof window !== "undefined"
    ? parseInt(new URLSearchParams(window.location.search).get("classId") ?? "")
    : undefined;
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className={`text-xs font-bold px-3 py-1.5 rounded-xl border
            ...
            : role === "parent"  ? "bg-emerald-50 text-emerald-600 border-emerald-100"
            : "bg-violet-50 text-violet-600 border-violet-100"}`}>
            {role === "student" ? "Student View" : role === "parent" ? "Parent View" : role === "teacher" ? "Teacher View" : "Admin View"}
          </span>
          {/* Real PDF download */}
          <DownloadButton
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-6 sm:p-8 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pb-6 border-b border-gray-100">
            <div className="flex items-start gap-4">
              <div className="w-16 h-16 rounded-2xl bg-indigo-50 flex items-center justify-center text-xl font-black text-indigo-600 shrink-0 overflow-hidden">
                {student.img
                  ? <img src={student.img} alt="" className="w-full h-full object-cover rounded-2xl" />
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { label: "Class Size",       value: `${classInfo.classSize} students` },
                { label: "Parent / Guardian", value: parent.name ? `${parent.name} ${parent.surname}` : "—" },
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="text-center px-3 py-3 text-[10px] font-black uppercase tracking-wider text-gray-400">Pos.</th>
                    ...
                    return (
                      <tr key={row.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-3 py-3 text-center">
                          <span className={`font-black text-sm ${band.color}`}>{row.totalScore.toFixed(1)}</span>
                        </td>
                        <td className="px-3 py-3 text-center"><GradeBadge grade={row.grade} /></td>
                        <td className="px-3 py-3 text-center"><PosBadge pos={row.position} /></td>
                        <td className="px-4 py-3 hidden lg:table-cell">
                          <span className="text-xs text-gray-400 italic">{row.remarks || band.label}</span>
                        </td>
                      </tr>
                    );
                  })}
                ...
                {subjectRows.length > 0 && (
                  <tfoot>
                    <tr className="bg-indigo-50 border-t-2 border-indigo-100">
                      ...
                      <td className="px-3 py-3 text-center font-black text-indigo-900">{overallStats.avgScore.toFixed(1)}%</td>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-9 gap-1.5">
            {[
              { g: "A1", r: "≥90", c: "bg-emerald-50 text-emerald-700 border-emerald-200" },
              { g: "B2", r: "≥80", c: "bg-teal-50 text-teal-700 border-teal-200"         },
              { g: "B3", r: "≥75", c: "bg-blue-50 text-blue-700 border-blue-200"         },
              { g: "C4", r: "≥70", c: "bg-sky-50 text-sky-700 border-sky-200"            },
              { g: "C5", r: "≥65", c: "bg-amber-50 text-amber-700 border-amber-200"      },
              { g: "C6", r: "≥60", c: "bg-yellow-50 text-yellow-700 border-yellow-200"   },
              { g: "D7", r: "≥55", c: "bg-orange-50 text-orange-700 border-orange-200"   },
              { g: "E8", r: "≥50", c: "bg-red-50 text-red-700 border-red-200"            },
              { g: "F9", r: "<50", c: "bg-rose-50 text-rose-700 border-rose-200"         },
            ].map((b) => (
              <div key={b.g} className={`text-center py-1.5 rounded-xl border text-[10px] font-black ${b.c}`}>
                <div>{b.g}</div><div className="opacity-60 font-semibold">{b.r}</div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Overall Average", value: `${overallStats.avgScore.toFixed(1)}%`, sub: `${overallStats.subjectCount} subjects`,        icon: <TrendingUp   size={15} />, color: "bg-indigo-50 text-indigo-600"  },
              { label: "Aggregate Score", value: overallStats.aggregate,                  sub: "Ghana BECE system",                            icon: <Award        size={15} />, color: "bg-amber-50 text-amber-600"   },
              { label: "Class Position",  value: ordinal(overallStats.overallPosition),   sub: `out of ${overallStats.classSize} students`,    icon: <Star         size={15} />, color: "bg-violet-50 text-violet-600" },
              { label: "Attendance Rate", value: `${attendanceRate}%`,                   sub: `${attendance.present} present · ${attendance.absent} absent`, icon: <CheckCircle2 size={15} />, color: attendanceRate >= 80 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600" },
            ].map((s) => (
          ...
          <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
            ...
            <div className="flex gap-4 text-[10px] font-bold text-gray-500">
              {[
                { label: `Present: ${attendance.present}`, color: "bg-emerald-500" },
                ...
                <span key={a.label} className="flex items-center gap-1">
                  <span className={`w-2 h-2 rounded-full inline-block ${a.color}`} />
                  {a.label}
                </span>
              ))}
            </div>
          ...
          <div className="p-5 rounded-2xl border-l-4 border-indigo-500 bg-indigo-50/50">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-indigo-400 mb-1">Performance Evaluation</p>
          ...
          {(role === "parent" || role === "student") && subjectRows.length > 0 && (() => {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-emerald-700"><strong>{best.name}</strong> — {best.grade} ({best.totalScore.toFixed(1)}%)</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-700"><strong>{worst.name}</strong> — {worst.grade} ({worst.totalScore.toFixed(1)}%)</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-3 gap-6 pt-6 border-t border-gray-100">
            {["Class Teacher", "Head Teacher / Principal", "Parent / Guardian"].map((label) => (
              <div key={label} className="flex flex-col gap-8">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 text-center -mt-6">{label}</p>
              </div>
            ))}
          </div>
