...
export default function BursarInviteModal() {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState(initialForm);
  const [result, setResult] = useState<BursarInviteCreateActionResult | null>(null);
  ...
  function submitInvite() {
    ...
    startTransition(async () => {
      const response = await createBursarInviteAction(form);
      setResult(response);
  ...
  const emailNote = result?.ok
    ? result.invite.emailWarning
      ? `Email delivery warning: ${result.invite.emailWarning}`
      : `Invite email handled by ${result.invite.emailProvider ?? "the configured provider"}.`
    : null;
  ...
  return (
    <>
      ...
      >
        ...
        <span className="hidden sm:inline">Invite bursar</span>
            ...
            <div className="max-h-[calc(100dvh-3rem)] overflow-y-auto p-4 pt-12 sm:p-8 sm:pt-8">
              <div className="mb-6">
                ...
                <p className="mt-2 text-sm font-medium leading-6 text-gray-500">
                  Create a secure finance invite. The bursar must sign in or sign up with this exact email before Edujay links them to this school.
                </p>
              </div>
