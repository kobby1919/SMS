...
import { reviewHomeworkSubmissionCorrectionRequest } from "@/src/lib/actions/actions";
import { useRouter } from "next/navigation";

export default function HomeworkCorrectionReviewActions({
  requestId,
}: {
  requestId: string;
}) {
  ...
  const review = (action: "APPROVE" | "REJECT") => {
    ...
    startTransition(async () => {
      try {
        const result = await reviewHomeworkSubmissionCorrectionRequest({
          requestId,
          action,
          note,
        });
        ...
        setError(caught instanceof Error ? caught.message : "Could not review homework correction.");
      } finally {
