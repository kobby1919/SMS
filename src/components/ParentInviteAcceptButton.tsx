...
import { acceptParentInviteAction } from "@/src/lib/actions/parentInviteActions";

export default function ParentInviteAcceptButton({ token }: { token: string }) {
  ...
  function acceptInvite() {
    ...
    startTransition(async () => {
      const result = await acceptParentInviteAction({ token });
      if (!result.ok) {
  ...
  return (
    <div className="mt-8">
      ...
      >
        ...
        {isPending ? "Connecting account..." : "Connect my parent account"}
      </button>
